...
void Application::prepare_cameras() {
    ...
    light_camera_projection = glm::mat4(glm::perspective(glm::radians(80.0f), 1.0f, 2.0f, 30.0f)); // <- modify this
...
void Application::update(float delta) {
    ...
    light_camera_view = glm::lookAt(
        glm::vec3(phong_lights_ubo.get_lights()[0].position),
        glm::vec3(phong_lights_ubo.get_lights()[0].spot_direction) + glm::vec3(phong_lights_ubo.get_lights()[0].position),
        glm::vec3(0.0f,1.0f,0.0f)); // <- modify this
    light_camera_data_ubo.set_view(light_camera_view);
    light_camera_data_ubo.update_opengl_data();
...
void Application::render() {
    ...
    case DISPLAY_VIEW_FROM_LIGHT: {
        ...
        light_camera_data_ubo.bind_buffer_base(CameraUBO::DEFAULT_LIGHTS_BINDING);
        render_scene(true, false);
        break;
    }
...
void Application::render_scene(bool from_light, bool gen_shadows) {
    ...
    if (!from_light) render_object(light_object, default_unlit_program);
}
